...
{
void exitLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  current_ = true;
  first_time_ = true;
  people_sub_ = nh.subscribe("/output/people/exit_zones", 1, &exitLayer::peopleCallback, this);
}

void exitLayer::peopleCallback(const people_msgs::People& people)
{
...
void exitLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
                               double* max_x, double* max_y)
{
  ...
  for (unsigned int i = 0; i < people_list_.people.size(); i++)
  {
    ...
    try
    {
      pt.point.x = person.position.x - 0.1;
      pt.point.y = person.position.y - 1.55;
