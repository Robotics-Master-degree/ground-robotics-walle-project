...
void safeLayer::onInitialize()
{
  ...
  current_ = true;
  first_time_ = true;
  people_sub_ = nh.subscribe("/output/people/danger_zones", 1, &safeLayer::peopleCallback, this);
  sub = nh.subscribe("/personaAgafada", 1, &safeLayer::enabledCallback, this);

}



void safeLayer::peopleCallback(const people_msgs::People& people)
{
  boost::recursive_mutex::scoped_lock lock(lock_);
  people_list_ = people;
}

void safeLayer::enabledCallback(std_msgs::Bool personaAgafada)
{
  boost::recursive_mutex::scoped_lock lock(lock_);
...
void safeLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
