...
#include <safe_navigation_layers/proxemic_layer.h>
#include <math.h>
...
PLUGINLIB_EXPORT_CLASS(safe_navigation_layers::ProxemicLayer, costmap_2d::Layer)
...
void ProxemicLayer::configure(ProxemicLayerConfig &config, uint32_t level)
{
  ...
  cutoff_ = config.cutoff;//96.14;
  amplitude_ = config.amplitude;//254;
