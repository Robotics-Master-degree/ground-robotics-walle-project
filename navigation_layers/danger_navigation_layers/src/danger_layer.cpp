...
#include <danger_navigation_layers/danger_layer.h>
#include <math.h>
...
{
void dangerLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  current_ = true;
  first_time_ = true;
  people_sub_ = nh.subscribe("/output/people/danger_zones", 1, &dangerLayer::peopleCallback, this);
  sub = nh.subscribe("/personaAgafada", 1, &dangerLayer::enabledCallback, this);

}



void dangerLayer::peopleCallback(const people_msgs::People& people)
{
  boost::recursive_mutex::scoped_lock lock(lock_);
  people_list_ = people;
}

void dangerLayer::enabledCallback(std_msgs::Bool personaAgafada)
{
  boost::recursive_mutex::scoped_lock lock(lock_);
  enabled_ = personaAgafada.data;
}


void dangerLayer::updateBounds(double origin_x, double origin_y, double origin_z, double* min_x, double* min_y,
