...
#include <danger_navigation_layers/proxemic_layer.h>
#include <math.h>
...
PLUGINLIB_EXPORT_CLASS(danger_navigation_layers::ProxemicLayer, costmap_2d::Layer)
  ...
  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end(); ++p_it)
  {
    ...
    for (int i = start_x; i < end_x; i++)
    {
      for (int j = start_y; j < end_y; j++)
      {
        ...
        cost_value = costmap_2d::LETHAL_OBSTACLE;
        costmap->setCost(i + dx, j + dy, cost_value);
...
void ProxemicLayer::configure(ProxemicLayerConfig &config, uint32_t level)
{
  ...
  cutoff_ = 10.0;//config.cutoff;//96.14;
  amplitude_ = 100;//config.amplitude;//254;
  covar_ =  0.26;// config.covariance;*/
