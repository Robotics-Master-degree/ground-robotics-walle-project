...
ros::Publisher pub_persones;
 ...
 geometry_msgs::PointStamped persones_points [];
      ...
      if ((centroidPointb.point.x != 0) and (color_def=="blue")){
        std::cout <<"PERSONAAA!!! (en funció camera_depth_optical_frame)  X: " << centroidPointb.point.x <<" Y: " << centroidPointb.point.y << " Z: " << centroidPointb.point.z << "\n";
         ...
         for (int i=0; i < 6; i++){
           ...
           if ((persones_points[i].point.x - punt_transformat.point.x < abs(0.3)) and (persones_points[i].point.y - punt_transformat.point.y < abs(0.3)))
           {
             existeix = true;
           }
         }

         if ((existeix == false) and (color_def == "blue") and (persones_points[i].point.x - punt_transformat.point.x < abs(0.5)) and (persones_points[i].point.y - punt_transformat.point.y < abs(0.5))){
           int k = persones_points.size();
           persones_points[k+1].point.x = punt_transformat.point.x;
           persones_points[k+1].point.y = punt_transformat.point.y;
           persones_points[k+1].point.z = punt_transformat.point.z;
         }
//
         }
      }
...
//CROPPING --Per eliminar soroll. Elimina punts que llegim de la càmera per centrar l'atenció en una àrea en concret.
    ...
    Eigen::Vector4f min_pt(-2.0, -1, 0.5, 1);
    Eigen::Vector4f max_pt(2.0, 1, 5, 1);
