...
bool find_rescued_person(geometry_msgs::Pose persona, geometry_msgs::PoseArray pers_salvades, double dist){
  ...
  for (int j=0; j<num_pers_salvades; j++){
    ...
    int dist_pers_ = calculate_euclidean_distance(x1,y1,x2,y2);
    if (abs(dist_pers_) < dist){
        trobat=true;
        std::cout << "aquesta persona ja ha estat tractada";
    }
 ...
 int cerca_persona_proxima (geometry_msgs::PoseArray pers_a_salvar, move_base_msgs::MoveBaseActionFeedback fb,geometry_msgs::PoseArray pers_salvades, double dist ){
        int num_persones_a_salvar = pers_a_salvar.poses.size();
        std::cout << "num_persones_a_salvar: " << num_persones_a_salvar;
        int num_persones_salvades = pers_salvades.poses.size();
        std::cout << "num_pers_salvades: " << num_persones_salvades;
        double dist_pers_a_salvar,x1,y1,x2,y2,x3,y3,dist_pers_salvades;
        x1 = fb.feedback.base_position.pose.position.x;
        y1 = fb.feedback.base_position.pose.position.y;
        //std::cout << "num_pers_a_salvar :"  << num_persones_a_salvar << std::endl;
        //std::cout << "num_pers_salvades :"  << num_persones_salvades << std::endl;
        int i_;
        double dist_ = 999;
        //mirem la distància de la persona a salvar amb la posició actual
        for (int i = 0; i < num_persones_a_salvar; i++) {
            x2 = pers_a_salvar.poses[i].position.x;
            y2 = pers_a_salvar.poses[i].position.y;
            dist_pers_a_salvar = calculate_euclidean_distance(x1, y1, x2, y2);
            std::cout << "dist_pers_a_salvar: " << dist_pers_a_salvar;
            if (num_persones_salvades == 0){
              ...
              if (dist_pers_a_salvar < dist_){
                ...
                bool trobat = find_rescued_person(pers_a_salvar.poses[i],pers_salvades,dist);
                if (!trobat){
     ...
     int cerca_persona_proxima_2 (geometry_msgs::PoseArray pers_a_salvar, move_base_msgs::MoveBaseActionFeedback fb,geometry_msgs::PoseArray pers_salvades, double dist ){
          ...
          //especifiquem punts start goal per cada persona
          //Posicio actual robot
            ...
            for (int i = 0; i < num_persones_a_salvar; i++) {
              ...
              srv.request.goal.pose.position.x = pers_a_salvar.poses[i].position.x -0.7;
              srv.request.goal.pose.position.y = pers_a_salvar.poses[i].position.y -0.7;
              ...
              std::cout << "Persona n"<< i << "   distancia check_path_a_salvar:  " << dist_pers_a_salvar;
...
void cerebro(MoveBaseClient& ac){
  ...
  if (personaagafada == false) { //No hi ha persona Agafada
    ...
    //CODI AQUI
      ...
      std::cout << "Case1: persona no agafada ." << std::endl;
      ...
      if (((persones_a_salvar_.poses.size()!= 0) and (persones_salvades_.poses.size() < 5)) and (persones_a_salvar_.poses.size()!=persones_salvades_.poses.size())){
        ...
        std::cout << "Persona trobada, anem a buscar persona: ";
        std::cout << num_persona_agafada << std::endl;
        ...
        while(!ac.waitForServer(ros::Duration(5.0)));
          ...
          goal.target_pose.pose.position.x =  persones_a_salvar_.poses[num_persona_agafada].position.x - 0.8;
          goal.target_pose.pose.position.y =  persones_a_salvar_.poses[num_persona_agafada].position.y - 0.8;
          ...
          if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
            ...
            if((ac.getState() == actionlib::SimpleClientGoalState::REJECTED ) or (ac.getState() == actionlib::SimpleClientGoalState::ABORTED)){
              ...
              try{
                goal.target_pose.header.frame_id = "map";
                goal.target_pose.header.stamp = ros::Time::now();
                goal.target_pose.pose.position.x =  feedback_.feedback.base_position.pose.position.x + 0.8;
                goal.target_pose.pose.position.y =    feedback_.feedback.base_position.pose.position.y + 0.8;
                goal.target_pose.pose.orientation.w = 1.0;
                goalid = feedback_.status.goal_id;
                std::cout << goalid << std::endl;
              //  ROS_INFO("Try to catch feedback from explorer");
          ...
          std::cout << "Case2: Encara no s'ha rebut persona, explorem";
      ...
      if(sortida_points[0].point.x != 0 and sortida_points[0].point.y != 0){
          ROS_INFO("Case 3: Coneixo Sortida i porto persona agafada");
  ...
  //Cancel·lo goal
          ...
          while(!ac.waitForServer(ros::Duration(5.0)));
            goal.target_pose.header.frame_id = "map";
            goal.target_pose.header.stamp = ros::Time::now();
            goal.target_pose.pose.position.x =   sortida_points[0].point.x - 1;
            goal.target_pose.pose.position.y =   sortida_points[0].point.y ;
            goal.target_pose.pose.orientation.w = 1.0;
            ROS_INFO("Anem a deixar persona  .................................... ");
            ac.sendGoal(goal);
            ac.waitForResult();
            if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
              ROS_INFO("Deixo persona  ...........................................");
              personaagafada = false;
              persones_salvades_.poses.push_back(persones_a_salvar_.poses[num_persona_agafada]);
              std::cout << "num_pers_salvades" << persones_salvades_;
              ...
              if((ac.getState() == actionlib::SimpleClientGoalState::REJECTED ) or (ac.getState() == actionlib::SimpleClientGoalState::ABORTED)){
                ...
                try{
                  goal.target_pose.header.frame_id = "map";
                  goal.target_pose.header.stamp = ros::Time::now();
                  goal.target_pose.pose.position.x =  feedback_.feedback.base_position.pose.position.x - 1.2;
                  goal.target_pose.pose.position.y =    feedback_.feedback.base_position.pose.position.y;
                  goal.target_pose.pose.orientation.w = 1.0;
                  goalid = feedback_.status.goal_id;
                  std::cout << goalid << std::endl;
                //  ROS_INFO("Try to catch feedback from explorer");
          ...
          //Publish exploration true
          ROS_INFO("No conec sortida, exploro");
          cmd_exploration.data = true;
          pub_cmdExploration.publish(cmd_exploration);
      }
  }
